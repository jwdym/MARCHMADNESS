...
Get_ELO <- function(scalar, df, teamid, iter, baseline, I, K){
  ...
  if((I > 1985) & (iter == 1)){
    newcheck <- c(names(ELO_LIST[[I - 1984]][K]) %in% names(ELO_LIST[[I-1985]][K]))
    if(!newcheck){
      return(baseline) 
    }
    else {
      elo <- mean(ELO_LIST[[I-1985]][[K]])
  ...
  we <- 1 / (10 ^(-dr / 25) + 1) # edited Probability estimation so estimated probabilities had a better range
...
for (i in 1985:1988){
  ...
  for (j in 1:n_game){
    ...
    for (k in 1:n_teams){
      ...
      ELO_LIST[[i-1984]][[k]][j] <- Get_ELO(scalar = 5, team = TEMP_LIST[[k]]$TeamID[1], df = TEMP_LIST, iter = j, baseline = 1000, I = i, K = k)
    }
