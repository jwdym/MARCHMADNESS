...
Get_ELO <- function(k, df, teamid, iter, baseline){
  if((i == 1985) & (iter == 1)){
    ...
    return(mean(ELO_LIST[[i-1984]][[k]])) ###THIS LINE IS A PROBLEM!!!!
  }
  else{
...
for (i in 1985:2007){
  ...
  for (j in 1:n_game){
    ...
    for (k in 1:n_teams){
      ...
      ELO_LIST[[i-1984]][[k]][j] <- Get_ELO(k = 5, team = TEMP_LIST[[k]]$TeamID[1], df = TEMP_LIST, iter = j, baseline = 1000)
    }
...
OK, so we have some Elos... but we're going to have to tune the Elo  
