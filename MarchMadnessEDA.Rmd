...
Get_ELO <- function(k, df, teamid, iter, baseline){
  if((i == 1985) & (iter == 1)){
    return(baseline)
  }
  else if((i > 1985) & (iter == 1)){
    return(mean(ELO_LIST[[i-1]][[k]])) 
  }
  else{
    tdat <- df[[which(Teams$TeamID == teamid)]][iter, ] #grabs teams previous elo
    Current_ELO <- ELO_LIST[[i]][[k]][iter - 1]
    ...
    Opponent_ELO <- ifelse(pos ==1, baseline, ELO_LIST[[i]][[which(Teams$TeamID == opid)]][pos - 1]) #finds the right row, then uses the previous row's elo score
...
for (i in 1985:2007){
  ...
  for(j in 1:n_teams){ELO_LIST[[i - 1984]][[j]] <- c()}
  n_game <- max(unlist(lapply(TEMP_LIST, nrow)))
  n_teams <- length(TEMP_LIST)
  for(j in 1:n_teams){ELO_LIST[[i - 1984]][[j]] <- nrow(TEMP_LIST[[j]])}
  for (j in 1:n_game){
    #code here 
    for (k in 1:n_teams){
      if(dim(TEMP_LIST[[k]])[1] < j) next
      # calculate elo for each team 
      con <- Get_ELO(k = 20, teamid = TEMP_LIST[[k]]$TeamID, df = TEMP_LIST, iter = j, baseline = 1000)
      ELO_LIST[[i-1984]][[k]][j] <- con 
    }
