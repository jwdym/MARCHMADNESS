...
for (i in 1985:2007){
  ...
  TEMP_LIST <- lapply(Team_ELO, szn_filter) #generates list of elos for that given year
  ...
  for (j in 1:n_game){
    ...
    for (k in 1:n_teams){
      ...
      ELO_LIST [[i - 1984]][[k]] <- c()
      #
      ELO_LIST [[i-1984]][[k]][j] <- Get_ELO(k = 20, team = TEMP_LIST[[k]]$TeamID, df = TEMP_LIST, iter = j, baseline = 1000) 
    }
