...
Get_ELO <- function(scalar, df, teamid, iter, baseline, I, K){
  ...
  dr <- Current_ELO + 100 * (tdat$Location == "H") - (Opponent_ELO + 200 * (odat$Location == "H"))
...
Prob_Predict <- function(teams, S){
  name1 <- Teams$TeamID[Teams$TeamID == teams[, 1]]
  name2 <- Teams$TeamID[Teams$TeamID == teams[, 2]]
  n <- length(name1)
  elo1 <- c()
  elo2 <- c()
  for(i in 1:n){
    elo1t <- Team_ELO[[names(Team_ELO) == name1[i]]]$ELO
    elo1[i] <- elo1t[length(elo1t)]
    elo2t <- Team_ELO[[names(Team_ELO) == name2[i]]]$ELO
    elo2[i] <- elo2t[length(elo2t)]
    
  }
  elo1 <- 
  dr <- elo1 + 200 * (tdat$Location == "H") - (elo2 + 200 * (odat$Location == "H"))
