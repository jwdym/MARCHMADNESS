...
Get_ELO <- function(k, df, teamid, iter){
  tdat <- df[[which(Teams$TeamID == teamid)]][iter - 1, ] #grabs teams previous elo
  ...
  Opponent_ELO <- ifelse(pos ==1, 1000, odat[pos - 1, ]$ELO) #finds the right row, then uses the previous row's elo score
